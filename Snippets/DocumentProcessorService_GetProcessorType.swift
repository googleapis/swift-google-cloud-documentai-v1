...
func sample(
  ...
  let response = try await client.getProcessorType(
    request: GetProcessorTypeRequest()
      .with {
        $0.name = "projects/\(projectId)/locations/\(locationId)/processorTypes/\(processorTypeId)"
      }
  )
